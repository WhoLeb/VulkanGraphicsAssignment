...
namespace assignment
{
	...
	{
		vkFreeDescriptorSets(device.device(), descriptorPool, uint32_t(descriptors.size()), descriptors.data());
	}
	...
	DescriptorWriter& DescriptorWriter::writeBuffer(uint32_t binding, VkDescriptorBufferInfo* bufferInfo)
	{
		...
		assert(bindingDescription.descriptorCount == 1 && "Dinding single descriptor info, but binding expects multiple");
